  ...
  ?>
	...
	<div class="large-8 large-centered columns">
		...
		if(isset($_GET['id'])) {
			...
			$q = "SELECT userID FROM user WHERE userID = '$vendorID'";
			$r = mysqli_query($dbc, $q);
			$row = mysqli_fetch_array($r);

			// Database - Remove user from the database
			include("includes/dbcon.php"); //returns $dbc
			$q2 = "DELETE FROM user WHERE userID = '$vendorID'";
			$r2 = mysqli_query($dbc, $q2);


				if ($r and $r2){
					echo "<h4>".$row['userID']." has been rejected and the account has been removed from the system.</h4>";
				} else {
